...
// ========== Display active battery + temperature + alert ==========
void showActiveBattery(String batteryName, int adcValue, float voltage) {
  ...
  // Alert when temperature exceeds 40°C
  if (currentTemp > 40.0) {
...
// ========== Update alert pins based on temperature ==========
void updateAlertPins() {
  if (currentTemp > 40.0) {
    digitalWrite(ALERT_PIN1, HIGH);
...
void loop() {
  ...
  while (millis() - startTime < B_ON_TIME) {
    if (millis() - lastMeasure >= MEASURE_INTERVAL) {
      ...
      updateAlertPins();           // Set alert pins based on temperature
